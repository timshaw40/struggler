...
# The self-play stack is opt-in (the `rl` extra): numpy + torch. Skip the
# module rather than failing collection, so `pip install -e ".[test]"` plus
# `pytest` still runs the rest of the suite on a plain checkout.
